...
# Visualize income distributions (optional)
...
# Create the reverse bias model
revbiasModel = myModel( { "gender": -1, "ethnic": -1 } ) 

# Simulate income evolution for world1 using the reverse bias model
world1_reversed = simulate_income_evolution(world1, revbiasModel)

# Analyze income distribution after 30 years in world1 with the reverse bias model
print("Income Distribution in World1 after 30 years (Reverse Bias Model):")
print(world1_reversed['income_final'].describe())
print(world1_reversed['income_final'].quantile([0.25, 0.5, 0.75]))  # Quartiles
...
sns.histplot(data=world1_reversed, x='income_final', kde=True, label='Reverse Bias Model')
plt.xlabel('Income after 30 years')
plt.ylabel('Density')
plt.title('Income Distribution in World1 after 30 years (Reverse Bias Model)')
plt.show()

# So yes, if we start off with World 1 on this revbiasModel, there a chance for the world to 
# eventual become fair like World #2 in 30 years, provided that the values of b_ethnic values are changed.
